...
suggest_chain = SUGGEST_PROMPT | llm | ListOutputParser()

async def suggest_question(state: GraphState) -> Dict[str, Any]:
    print("---KEYWORD EXTRACT---")
    question = state["question"]
    context = state["rerank_context"]
    answer = state['answer']

    suggests = await suggest_chain.ainvoke({"context": context, "question": question, "answer": answer})
