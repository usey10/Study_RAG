...
keyword_chain = KEYWORD_PROMPT | llm | ListOutputParser()

async def keyword_extract(state: GraphState) -> Dict[str, Any]:
    print("---KEYWORD EXTRACT---")
    question = state["question"]
    context = state["rerank_context"]
    answer = state['answer']

    keywords = await keyword_chain.ainvoke({"context": context, "question": question, "answer": answer})
